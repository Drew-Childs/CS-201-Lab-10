...
{
	string name;
	cout << "Insert student's name to add ==> ";
	cin >> name;
	Node * curr = head_ptr;

	while (curr != nullptr) {
		if ((curr->next == nullptr) && (curr == head_ptr)) {
			curr->next = new Node(name, nullptr);
			break;
		}
		curr = curr->next;
...
{
	string name;
	cout << "Insert student's name to delete ==> ";
	cin >> name;
	Node * curr = head_ptr;

	while (curr->next != nullptr) {
		if (((curr->next->name == name) && (curr->next->next == nullptr)) || (curr->next->name == name)) {
...
void insertNode(Node*& head_ptr)
{
	...
	while (current_ptr != nullptr) {
		if ((linkedListPosition == insertPosition) && (linkedListPosition == 0)) {
			Node * newNode = new Node(insertName);
			newNode->next = head_ptr;
			head_ptr = newNode;
		}
		if (linkedListPosition == insertPosition - 1) {
